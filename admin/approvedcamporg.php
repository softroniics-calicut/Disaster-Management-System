...
$details=$_GET['update_id'];
 ...
 $mail_id=mysqli_query($conn,"SELECT email_id FROM register_tb WHERE login_id='$details' ");
   $row=mysqli_fetch_assoc($mail_id);
   $to_address=$row['email_id'];
    // var_dump($to_address);exit();


   mysqli_query($conn,"UPDATE register_tb SET approve_status='1' WHERE login_id='$details' ");
